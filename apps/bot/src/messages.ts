...
export function howItWorksMessage() {
  return [
    ...
    "2. Вы выбираете формат и шаблон.",
    "3. Бот сам выбирает сильный CTR-текст под ролик и шаблон.",
    "4. Вы загружаете фото, лицо или кадр-основу.",
...
export function platformPrompt() {
  return "Выберите формат обложки. Вариант «Без лица» — отдельный тип обложки в формате 16:9.";
}
...
export function referenceForGenerationPrompt() {
  return [
    ...
    "",
    "От него я возьму лицо, позу или композицию и сразу начну создание.",
    "Спишется 1 генерация. При технической ошибке баланс вернётся."
  ].join("\n");
}
...
export function termsMessage() {
  return [
    ...
    "3. Покупки цифровых услуг оплачиваются в рублях через платёжную ссылку.",
